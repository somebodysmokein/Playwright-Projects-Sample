...
/* test('read session from context', async ({ page, context }) => {
  // cookies (all domains)
  
  // localStorage: ensure page is at correct origin first
...
test('sample test 2', async ({ page }) => {
    ...
    const project = info.project?.name || process.env.PW_PROJECT || 'accountA';
